...
run_segment() {
	...
	if [ "$vcs_type" = "git" ] && git rev-parse --git-dir >/dev/null 2>&1; then
		branch=$(__parse_git_branch "$vcs_rootpath")
	elif [ "$vcs_type" = "hg" ] && hg root >/dev/null 2>&1; then
		branch=$(__parse_hg_branch "$vcs_rootpath")
	elif [ "$vcs_type" = "svn" ] && svn info >/dev/null 2>&1; then
		branch=$(__parse_svn_branch "$vcs_rootpath")
	else
		branch="#[fg=${TMUX_POWERLINE_CUR_SEGMENT_FG}]"
	fi
...
__parse_git_branch() {
	...
	if [ -n "$branch" ]; then
		echo -n "#[fg=colour${TMUX_POWERLINE_SEG_VCS_BRANCH_GIT_SYMBOL_COLOUR}]${icon} #[fg=${TMUX_POWERLINE_CUR_SEGMENT_FG}]${branch}"
	else
		echo -n "#[fg=colour${TMUX_POWERLINE_SEG_VCS_BRANCH_GIT_SYMBOL_COLOUR}]${icon}"
	fi
}

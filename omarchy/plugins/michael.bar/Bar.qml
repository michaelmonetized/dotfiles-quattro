...
Item {
  ...
  // Injected by the host after load. Must not be `required` — plugin bars
  // are created by an async Loader, which cannot supply these at construct time.
  ...
  property color chipForeground: "#11111b"
  ...
  component ModuleSlot: Item {
    ...
    readonly property real contentWidth: activeItem && activeItem.visible
      ...
      : 0
    readonly property real contentHeight: activeItem && activeItem.visible ? activeItem.implicitHeight : 0
    readonly property real leadWidth: !root.vertical && leadLabel.visible ? leadLabel.implicitWidth : 0
    readonly property real joinWidth: !root.vertical && joinLabel.visible ? Math.max(0, joinLabel.implicitWidth - 1) : 0
    implicitWidth: leadWidth + contentWidth + joinWidth
    implicitHeight: Math.max(contentHeight, (leadLabel.visible || joinLabel.visible) ? root.barSize : 0)
    width: implicitWidth
    ...
    readonly property string chipColorValue: String(moduleSettings.chipColor || "")
    readonly property bool hasChip: chipColorValue.length > 0
    readonly property string chipRadiusValue: String(moduleSettings.chipRadius || "")
    readonly property string leadGlyph: String(moduleSettings.chipLead || "")
    readonly property string joinGlyph: String(moduleSettings.chipJoin || "")
    readonly property string joinFont: String(moduleSettings.chipFont || "NotoSansM Nerd Font Mono")
    readonly property bool roundLeft: chipRadiusValue === "left" || chipRadiusValue === "all"
    readonly property bool roundRight: chipRadiusValue === "right" || chipRadiusValue === "all"
    ...
    Rectangle {
      ...
      topLeftRadius: slot.roundLeft ? height / 2 : 0
      bottomLeftRadius: slot.roundLeft ? height / 2 : 0
      topRightRadius: slot.roundRight ? height / 2 : 0
      bottomRightRadius: slot.roundRight ? height / 2 : 0
    }

    Text {
      id: leadLabel
      visible: !root.vertical && slot.hasChip && slot.leadGlyph !== ""
      x: 0
      y: 0
      z: 2
      height: parent.height
      text: slot.leadGlyph
      color: slot.chipColorValue
      font.family: slot.joinFont
    ...
    Text {
      id: joinLabel
      visible: !root.vertical && slot.hasChip && slot.joinGlyph !== ""
      x: slot.leadWidth + slot.contentWidth - 1
      y: 0
      z: 2
      height: parent.height
      text: slot.joinGlyph
    ...
    function injectProps() {
      ...
      Qt.callLater(applyChipForeground)
    }

    function isPopupSurface(item) {
      if (!item) return false
      return ("anchorItem" in item && "contentWidth" in item) || item.triggerMode !== undefined
    }

    function applyChipForeground() {
      if (!slot.hasChip || !slot.activeItem) return
      var tint = root.chipForeground
      function paint(item) {
        if (!item || slot.isPopupSurface(item)) return
        if (item.foreground !== undefined) item.foreground = tint
        var kids = item.children
        if (!kids) return
        for (var i = 0; i < kids.length; i++) paint(kids[i])
      }
      paint(slot.activeItem)
    }
  ...
  component CustomCommandModule: WidgetButton {
    ...
    foreground: {
      var c = String(setting("color", ""))
      if (c.length > 0) return c
      if (String(setting("chipColor", "")) !== "") return root.chipForeground
